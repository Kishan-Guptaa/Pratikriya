    ...
    <section className="py-12 md:py-16 px-6 relative bg-primary/5 dark:bg-primary/10 overflow-hidden">
      <div className="max-w-4xl mx-auto text-center">
        ...
        <div className="relative glass-panel rounded-3xl p-4 md:p-6 border-4 border-foreground shadow-[6px_6px_0px_#2d2638] dark:shadow-[6px_6px_0px_#f5f3f7] rotate-1 max-w-3xl mx-auto z-10">
          ...
          <div className="flex flex-col md:flex-row gap-4 text-left h-[340px]">
            {/* Fake Sidebar */}
            <div className="w-full md:w-44 bg-white/50 dark:bg-black/50 rounded-xl p-3 border-2 border-foreground/20 flex flex-col gap-2">
              <div className="h-8 rounded-lg bg-primary/20 flex items-center px-3 gap-2">
                <Search className="size-3.5" /> <div className="h-1.5 w-12 bg-foreground/20 rounded" />
              </div>
              <div className="mt-2 space-y-2 flex-1">
                <div className="h-8 rounded-lg bg-white dark:bg-black border-2 border-foreground shadow-sm flex items-center px-3 text-[10px] font-bold gap-2">
                  <span className="text-primary font-bold">Aa</span> Short Text
                </div>
                <div className="h-8 rounded-lg bg-white/50 dark:bg-black/50 border border-dashed border-foreground/50 flex items-center px-3 text-[10px] gap-2">
              ...
              <div className="h-8 rounded-lg bg-primary text-white flex items-center justify-center font-bold font-patrick-hand text-base mt-auto hover:scale-105 transition-transform cursor-pointer">
                <Plus className="size-4 mr-1.5" /> Publish
              </div>
            </div>

            {/* Fake Canvas */}
            <div className="flex-1 bg-slate-50 dark:bg-[#130f1d] rounded-xl p-4 border-2 border-foreground/20 overflow-hidden relative">
