...
import { useSignup, useSignIn } from "~/hooks/api/auth"
...
export function SignupForm({
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (msg.toLowerCase().includes("already exists")) {
        setSignupError("A user with this email already exists. Please login instead!");
      } else {
  ...
  const handleMockOAuth = async (provider: 'google' | 'github') => {
    ...
    try {
      ...
      console.error(err);
      setSignupError(`Failed to authenticate with ${provider === 'google' ? 'Google' : 'GitHub'}.`);
    } finally {
      setIsSubmitting(false);
    }
  }
          ...
          >
             <FieldGroup className="gap-3.5">
              <div className="flex flex-col items-center gap-1 text-center mb-3">
                <h1 className="text-3xl font-bold font-caveat tracking-wider text-slate-800">Create Account</h1>
                <p className="text-sm font-sans font-semibold text-slate-500 text-balance">
                  Join Pratikriya and start building!
                </p>
              </div>
              ...
              <Field className="gap-1.5">
                ...
                <Input id="name" type="text" placeholder="John Doe" required {...register("name")} className="h-10 text-base" />
              </Field>
              <Field className="gap-1.5">
                <FieldLabel htmlFor="email" className="text-sm">Email</FieldLabel>
                <Input id="email" type="email" placeholder="m@example.com" required {...register("email")} className="h-10 text-base" />
              </Field>
              <Field className="gap-1.5">
              ...
              <Field>
                <Button 
                  ...
                  className="scribble-btn bg-pastel-green hover:bg-green-200 text-[#2d2638] h-9 w-full text-base mt-1 flex items-center justify-center gap-1.5"
                >
                  {isSubmitting ? (
                    <>
              ...
              <FieldSeparator className="text-xs *:data-[slot=field-separator-content]:bg-white py-1">Or continue with</FieldSeparator>
              ...
              <FieldDescription className="text-center font-semibold mt-1 text-slate-600 text-sm">
                Already have an account? <a href="/login" className="text-[#7b61ff] underline font-bold">Sign in</a>
              </FieldDescription>
             </FieldGroup>
          </form>
        </CardContent>
